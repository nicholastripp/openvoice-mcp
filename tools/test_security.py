            ...
            async with session.get(
                ...
                for header in bad_headers:
                    if header in resp.headers:
                        ...
                        print(f"✅ {header} correctly removed")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(
                ...
                if csrf_cookie:
                    print(f"✅ CSRF cookie set: {csrf_cookie.value[:10]}...")
                else:
                    print("❌ No CSRF cookie received")
            ...
            async with session.post(
                ...
                if resp.status == 403:
                    ...
                    print(f"❌ POST without CSRF accepted (status: {resp.status})")
        ...
        try:
            async with session.post(
                ...
                if resp.status == 413:
                    ...
                    print(f"❌ Large request accepted (status: {resp.status})")
        except aiohttp.ClientPayloadError:
            print("✅ Large request rejected by client (payload too large)")
        except Exception as e:
            print(f"✅ Large request rejected: {type(e).__name__}")
...
async def check_file_permissions():
    ...
    if env_file.exists():
        ...
        if mode == 0o600:
            print(f"✅ .env has secure permissions: {oct(mode)}")
        else:
            print(f"❌ .env has insecure permissions: {oct(mode)} (should be 0o600)")
    else:
    ...
    if config_dir.exists():
        ...
        if not (mode & 0o077):  # No permissions for others
            print(f"✅ config directory has secure permissions: {oct(mode)}")
        else:
            print(f"⚠️  config directory has broad permissions: {oct(mode)}")
...
async def main():
    ...
    try:
        ...
        print("\n❌ Could not connect to web UI at https://localhost:8443")
